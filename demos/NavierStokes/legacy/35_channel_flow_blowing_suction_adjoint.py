...
NB_ITER = 15    ## 50 works for 360 nodes (lc=0.2, ref_io=2, ref_bs=5)
...
def simulate_adjoint_navier_stokes(cloud_lamb, 
    ...
    for i in range(NB_ITER):
        ...
        bc_mu = {"Wall":zero, "Inflow":zero, "Outflow":u[out_nodes_vel]*l1[out_nodes_lamb] + grad_l1/Re, "Blowing":zero, "Suction":zero}
        ...
        # grad_l1 = gradient_vals_vec(cloud_lamb.sorted_nodes[out_nodes_lamb], l1, cloud_lamb, RBF, MAX_DEGREE)[:, 0]
        # grad_l1_ = interpolate_field(grad_l1, cloud_lamb, cloud_mu)
        # grad_l1_ = interpolate_field(grad_l1, cloud_lamb, cloud_mu)

        # pi_ = pi_.at[out_nodes_pi].set(u[out_nodes_vel]*l1[out_nodes_lamb] + grad_l1/Re)
...
LR = 1e-1
GAMMA = 0.995
EPOCHS = 2000     ## 3 More than enough for 50 iter and 360 nodes, but at least 4 needed for 314 nodes
