...
def cost_val_fn(u, v):
    ...
    # term1 = 0.5 * jnp.trapz(integrand, x=y_out)
    # term2 = jnp.mean((u_out[:-1]-u_out[1:])**2)
    # return term1 + 100*term2

## A penalty term to make the solution smooother
def pen_grad_u(u):
    gradu = gradient_vals_vec(cloud_vel.sorted_nodes[out_nodes_vel], u, cloud_vel, RBF, MAX_DEGREE)[...,1]
    return 0.5 * jnp.trapz(gradu**2, x=y_out)
...
def loss_fn(u_inflow):
    ...
    # return cost_val_fn(u_list[-1], v_list[-1]) + 0.1*pen_grad_u(u_list[-1])
